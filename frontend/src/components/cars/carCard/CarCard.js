...
function CarCard({ car, className }) {
    const picture = car.picture
        ? <img alt={`Car n°${car.id}`} src={`data:image/jpeg;base64,${car.picture}`} className='ActionArea-PictureBox-Picture' />
        : <DefaultCarPicture className='ActionArea-PictureBox-Picture ActionArea-PictureBox-Picture_default' />;

    const modelTitleClassName = car.brand || car.model
        ? 'ActionArea-TitleBox-ModelSubTitle'
    ...
    return (
        <Card id='CarCard' className={className}>
